...
/*static*/ bool NtJPGLoader::LoadTextureDataFromFile(const ntWchar* fileName, std::unique_ptr<ntUchar[]>& outBuffer, ntUchar** bitData, ntUint& maxSize, ntUint& width, ntUint& height, ntSize& textureSize, ntUint& bpp)
{
	...
	if (false == NtSystem::LoadFile(fullPath, outBuffer, maxSize))
	{
		NTRACE(L"can't open %s\n", fileName);
		return false;
	}
	
	if (maxSize == 0)
	{
		NTRACE(L"read Buffer size if Zero!");
		return false;
	}
	...
	jpeg_mem_src(&cinfo, outBuffer.get(), maxSize);
	...
	*bitData = new ntUchar[width * height * cinfo.output_components];
	...
	// below code is Change for BRG Color Mode
	NTRACE("color Type : %d\n", cinfo.out_color_space);

	JSAMPROW srcPtr = *bitData;
	while (cinfo.output_scanline < cinfo.output_height) {
	   ...
	   */
		...
		(void) jpeg_read_scanlines(&cinfo, &readbuffer, 1);

		Crt::MemCpy(srcPtr, readbuffer, cinfo.output_width * cinfo.output_components);

		srcPtr += (cinfo.output_width * cinfo.output_components);
	}
